...
from ._utils import vector_to_angles, angles_to_vector
...
class StereographicProjection:
    ...
    def plot(self, ax=None, cmap="inferno", cmap_norm=plt.Normalize, vmin=None, vmax=None, 
             label="", s=50, add_ticks=True, draw_cross=True, add_radial_grid=True, 
    ...
    vmin : float, optional
        Minimum data value for colormap normalization. If None, the minimum value of the data is used.
    vmax : float, optional
        Maximum data value for colormap normalization. If None, the maximum value of the data is used.
    label : str, optional
    ...
    facecolor : str, optional
        The facecolor of the plot background. Default is transparent ("#ffffff00").
    ...
    """
        ...
        if add_radial_grid:
            ...
            for circ_inc in range(0, 90, 10): # Isoinclination lines every 10 degrees
                circ_dec = np.linspace(0, 360, 1000)  # Declinations from 0 to 360 degrees
        ...
        scatter_neg = ax.scatter(XY_projected[:, 1][~positive_inc], XY_projected[:, 0][~positive_inc],
                                 c="#ffffff00", edgecolors=colors[~positive_inc], label=f"{label}$I \leq 0$",
                                **kwargs)
